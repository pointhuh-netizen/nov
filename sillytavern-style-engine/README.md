...
├── axes/
│   ├── axis-a-pov.json            ← A축: 시점·서술방식. A-00~A-09
...
| **1계층** | **A** | 시점·서술방식. 서사 모드(동화, 무협, 서간체 등)를 포함한 복합 문체도 A축이 담당 | MUTEX |
...
4. **A축** 모듈 적용 (시점·서술방식 — 동화/무협/서간체 등 서사 모드 포함)
5. **B축** 모듈 적용 (어조·어휘)
6. **C~G축** 모듈 적용 (각 차원 미세 조정)
...
| 축 | 건드리는 슬롯 | 위치 |
|---|---|---|
| **A** | `{A_VOICE_POV_RATIO}`, `{A_VOICE_FID_STYLE}`, `{A_VOICE_STYLE_REF}`, `{A_PROSE_RHYTHM}`, `{A_PROSE_SENSORY}`, `{A_PROSE_DESCRIPTION}`, `{A_PROSE_CAUSALITY}`, `{A_PROSE_TIME_DENSITY}`, `{A_PROSE_PACING}`, `{A_PROSE_DWELL}`, `{A_PROSE_MEDITATIVE}`, `{A_PROSE_PHYSICAL_DISTANCE}` + 서사 모드는 `{B_*}`, `{D_*}`, static_overrides, preamble_overrides 추가 | MODULE_1_VOICE, MODULE_2_PROSE |
...
A~G축은 **단일 차원** 조정이므로, 자기 접두사 슬롯만 건드린다. 단, A-07(동화적 서술), A-08(무협적 서술), A-09(서간체) 같은 서사 모드 모듈은 B축/D축 슬롯, `static_overrides`, `preamble_overrides`, `persona` 등을 함께 교체한다:
...
- 모든 A축 모듈(동화적 서술, 무협적 서술, 서간체 등)에서 동일하게 적용되어야 함
