...
├── axes/
│   ├── axis-n-narrative-mode.json ← N축: 복합 문체 (서사 모드). 1계층. N-00~N-03
...
- [ ] **[LOW] N-04 극본체 추가**: 대사 중심 서사 모드
- [ ] **[LOW] N-05 웹소설체 추가**: 장르 관습 기반 복합 모드
...
| 현재 | **B축 전면 재작성**: B-00~B-07을 `operations` + `check_operations` + `master_guide_delta` + `known_conflicts` 표준 포맷으로 재작성. `override_mode`/`preamble_overrides`/`module_overrides` 비표준 구조 제거. `{B_TONE_TEMP}`, `{B_TONE_VOCAB}`, `{B_TONE_FID_GRAMMAR}`, `{B_TONE_ENDING}`, `{B_TONE_DIALOGUE_TAG}` 슬롯 연결. catalog.json에 B-00~B-07 등록 |
